...
```{r}
# clusters & complexes
clst_annot_file <- paste0(workdir, "ppi_ml/results/ppi_clustering/leca_clustering_cmplx_annots_011023.csv") 
...
clst_char <- clst_annot %>%
  ...
  tally() %>%
  arrange(desc(n))  # 01/10/23: still 1373 NAs out of 2778 
...
```{r}
# cmplx labels
cmplx_labels <- c("60S cytosolic large ribosomal subunit", "40S cytosolic small ribosomal subunit", "Nuclear pore complex", "Mitochondrial respiratory chain complex I", "IFT-B complex", "IFT-A complex", "19S Proteasome", "26S Proteasome complex", "CCT complex", "ESCRT-III complex", "Exosome complex", "COP9 signalosome complex", "TRAPP complex", "Arp2/3 protein complex", "COPI vesicle coat complex")

# get cut cols
cuts <- names(clst_annot[grepl("cut", names(clst_annot))])
cuts_sel <- cuts[c(2,4,6)]

# filter data
pdf <- clst_annot %>%
  filter(!is.na(characterization_status)) %>%
  mutate(characterization_status_fmt = case_when(characterization_status == "Known" ~ "Known interactions",
                                             characterization_status == "Novel member" ~ "Novel interaction partner",
                                             characterization_status == "Uncharacterized" ~ "Uncharacterized complex"))
```
...
circlepack_fxn <- function(graph_annot, seed){
  ...
  base_layout_circlepack_format <- base_layout_circlepack +
    ...
    scale_fill_manual(values = c("#E64B35", "#4DBBD5", "#F0E442", "#3C5488", "#00A087"), 
                      na.translate=F) + # node fill colors
    scale_color_manual(values = c("grey50") ,na.value = "grey50") +
    scale_size_manual(values = c(2), na.value = 0.5) + # set carefully; big nums freeze RStudio
    theme_void() +
    theme(legend.title = element_blank(),
          legend.position = "top") +
    geom_node_text(aes(label = genes_fmt), size = 1.5, color = "black")
...
ggsave(paste0(figure_dir, "cplot_annot-status_011023.pdf"), device = "pdf", 
              width = 6, height = 6, units = "in")
ggsave(paste0(figure_dir, "cplot_annot-status_011023.png"), device = "png", 
              width = 6, height = 6, units = "in")
```
