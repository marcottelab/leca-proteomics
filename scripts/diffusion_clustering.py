...
def main():
    ...
    #Test comment out
    try:
        igraph = importr('igraph', robject_translations = {'.env': '_env2'})
        dendextend = importr('dendextend', robject_translations = {'.env': '_env2'})
        dplyr = importr('dplyr', robject_translations = {'.env': '_env2'}, on_conflict="warn")
    ...
    cut_dend <- function(dendrogram, cuts){
        ...
        print("Get height of dendrogram")
   
        #node_attr <- get_nodes_attr(dendrogram, "height")

        ht <- attr(dendrogram,"height")
