...
def main():
    ...
    parser.add_argument("--pepcount", action="store_true", dest="pepcount", required=False, default=False,
                        help="If .pepcount, skip the first row")
    ...
    for i, filename in enumerate(args.prot_count_files):
        ...
        if args.pepcount:
           df = pd.read_table(filename, skiprows= 1, index_col = 0)
        else:
