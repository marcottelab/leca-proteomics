...
slc_levels <- pull(data, slc) %>% unique()
...
# bar chart
p3 <- ggplot(mapping_fmt, aes(x=slc, fill=fct_rev(leca_label))) +
  geom_bar(width=0.7, position="fill", size=1.3, color="black") +
  theme_bw(base_size = 14) +
...
panel %>% ggsave("localization_ml/figures/leca_uniprot_counts_species_101723.png", ., device = "png", width = 12, height = 6, units = "in")
panel %>% ggsave("localization_ml/figures/leca_uniprot_counts_species_101723.pdf", ., device = "pdf", width = 12, height = 6, units = "in")
